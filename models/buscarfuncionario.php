...
if (isset($_GET['term'])) {
    ...
    if (isset($_SESSION['codigoEmpresa'])) {
        ...
        $stmt = $conn->prepare("SELECT nome_funcionario FROM cad_funcionarios WHERE nome_funcionario LIKE ? AND cod_empresa = ? LIMIT 10");
        ...
        if (count($funcionarios) > 0) {
            // Se foram encontrados funcionários, retorna os nomes dos funcionários
            echo json_encode($funcionarios);
        } else {
            // Se nenhum funcionário foi encontrado, retorna uma mensagem indicando que nenhum funcionário foi encontrado
